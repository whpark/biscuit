...
#define SRC(sig) COMPOSE2(sig, SRC_TEXT)
	...
	SECTION("iconv_wrapper") {
		...
		unsigned char str[] = { 0xb0, 0xa1, 0xb3, 0xaa, 0xb4, 0xd9, 0xb6, 0xf3, 0xb8, 0xb6, 0xb9, 0xd9, 0xbb, 0xe7, 0x8c, 0x63 };
	...
	SECTION("iconv_wrapper") {
		...
		REQUIRE((bool)r);
		{
			...
			REQUIRE(str[1] == ComposeKSSM(Ini_N, Mid_R, Fin_Fill));	// '나'
			REQUIRE(str[2] == ComposeKSSM(Ini_D, Mid_R, Fin_Fill));	// '다'
			REQUIRE(str[3] == ComposeKSSM(Ini_L, Mid_R, Fin_Fill));	// '라'
			REQUIRE(str[4] == ComposeKSSM(Ini_M, Mid_R, Fin_Fill));	// '마'
			REQUIRE(str[5] == ComposeKSSM(Ini_B, Mid_R, Fin_Fill));	// '바'
			REQUIRE(str[6] == ComposeKSSM(Ini_S, Mid_R, Fin_Fill));	// '사'
			REQUIRE(str[7] == ComposeKSSM(Ini_DD, Mid_U, Fin_M));	// '똠'
			REQUIRE(str[0] == ComposeKSSM(L'ㄱ', L'ㅏ'));	// '가'
			REQUIRE(str[1] == ComposeKSSM(L'ㄴ', L'ㅏ'));	// '나'
			REQUIRE(str[2] == ComposeKSSM(L'ㄷ', L'ㅏ'));	// '다'
			REQUIRE(str[3] == ComposeKSSM(L'ㄹ', L'ㅏ'));	// '라'
			REQUIRE(str[4] == ComposeKSSM(L'ㅁ', L'ㅏ'));	// '마'
			REQUIRE(str[5] == ComposeKSSM(L'ㅂ', L'ㅏ'));	// '바'
			REQUIRE(str[6] == ComposeKSSM(L'ㅅ', L'ㅏ'));	// '사'
			REQUIRE(str[7] == ComposeKSSM(L'ㄸ', L'ㅗ', L'ㅁ'));	// '똠'
		}
