...
namespace biscuit::seq::test {
	...
	using seq_map_t = biscuit::seq::TSequenceMap<seq_t::result_t>;
	...
	protected:
		...
		coro_t Task1(seq_t& seq, param_t param) {
			...
			fmt::print("{}: End {}\n", funcname, ms(biscuit::seq::clock_t::now() - t0));
		...
		coro_t Task2(seq_t& seq, param_t param) {
			...
			auto str = fmt::format("{}: End : {}", funcname, ms(biscuit::seq::clock_t::now() - t0));
			fmt::print("{}\n", str);
