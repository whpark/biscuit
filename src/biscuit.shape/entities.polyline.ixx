	...
	public:
		...
		virtual bool UpdateBounds(rect_t& rectBoundary) const override {
			...
			for (int iPt = 0; iPt < nPt; iPt++) {
				auto pt0 = m_pts[iPt];
				bModified |= rectBoundary.UpdateBounds(pt0);
				if (pt0.Bulge() != 0.0) {
					auto iPt2 = (iPt+1) % m_pts.size();
					auto pt1 = m_pts[iPt2];
					xArc arc = xArc::GetFromBulge(pt0.Bulge(), pt0, pt1);
					bModified |= arc.UpdateBounds(rectBoundary);
				}
			}
		...
		static std::unique_ptr<xPolyline> MergeShapeAsPolyline(t_iterator begin, t_iterator end, double dThreshold) {
			...
			for (auto pos = begin; pos != end; pos++) {
				...
				if (shape.GetShapeType() == eSHAPE::line) {
					...
					pts.back().Bulge() = units::angle::rad_t(arc.m_angle_length)/4.;
...
export CEREAL_REGISTER_TYPE(biscuit::shape::xPolyline);
export CEREAL_REGISTER_POLYMORPHIC_RELATION(biscuit::shape::xShape, biscuit::shape::xPolyline);
export CEREAL_CLASS_VERSION(biscuit::shape::xPolyline, biscuit::shape::xPolyline::s_version);

export CEREAL_REGISTER_TYPE(biscuit::shape::xPolylineLW);
export CEREAL_REGISTER_POLYMORPHIC_RELATION(biscuit::shape::xShape, biscuit::shape::xPolylineLW);
export CEREAL_CLASS_VERSION(biscuit::shape::xPolylineLW, biscuit::shape::xPolylineLW::s_version);
